...
from partsgenie.utils import codon_utils, dna_utils, seq_utils
from partsgenie.optim.sim_ann import SimulatedAnnealer
from partsgenie import vienna_utils, rbs_calculator
...
class PartsSolution():
    ...
    def get_values(self):
        ...
        params = self.__dna.get('temp_params', None)
    ...
    def mutate(self):
        ...
        for feature in self.__dna_new['features']:
            if feature['typ'] == dna_utils.SO_CDS \
                ...
                feature['seq'] = self.__cod_opt.mutate(
                    feature['temp_params']['aa_seq'].upper(),
                    feature['seq'].upper(),
                    mutation_rate)
            elif not feature['temp_params']['fixed']:
                feature['seq'] = seq_utils.mutate_seq(feature['seq'].upper(),
                                                      mutations=3)
    ...
    def __calc_num_inv_seq_fixed(self, fixed_seqs):
        ...
        self.__dna['temp_params'] = self.__dna.get('temp_params', {})

        self.__dna['temp_params']['num_inv_seq_fixed'] = \
            sum(len(seq_utils.find_invalid(
                seq,
    ...
    def __calc_num_rogue_rbs_fixed(self):
        ...
        for idx, feature in enumerate(self.__dna['features']):
            if feature['typ'] == dna_utils.SO_RBS:
                prefix = self.__dna['features'][idx - 1]['seq'] \
                    if idx > 0 else ''
                cds = self.__dna['features'][idx + 1]

                if cds['temp_params']['fixed']:
                    _, rogue_rbs = self.__calc_tirs(prefix, feature, cds)
    ...
    def __init_seqs(self):
        ...
        for idx, feature in reversed(list(enumerate(self.__dna['features']))):
            feature['parameters'] = feature.get('parameters', {})

            if feature['typ'] == dna_utils.SO_CDS \
                    and not feature['temp_params']['fixed']:
                if feature['temp_params'].get('stop_codon', True):
                    feature['temp_params']['aa_seq'] += '*'

                feature['links'] = ['http://identifiers.org/taxonomy/' +
                                    self.__organism['taxonomy_id']]

                feature['seq'] = self.__cod_opt.get_codon_optim_seq(
                    feature['temp_params']['aa_seq'],
    ...
    def __update(self, dna):
        ...
        for idx, feature in enumerate(dna['features']):
            if feature['typ'] == dna_utils.SO_RBS:
                prefix = dna['features'][idx - 1]['seq'] if idx > 0 else ''
                cds = dna['features'][idx + 1]
                tir_err, rogue_rbs = self.__calc_tirs(prefix, feature, cds)
        ...
        dna['parameters'] = dna.get('parameters', {})
    ...
    def __calc_tirs(self, prefix, rbs, cds):
        """Performs TIR calculations."""
        pre_cds_seq = prefix + rbs['seq']

        tir_vals = self.__calc.calc_dgs(pre_cds_seq + cds['seq'],
                                        len(pre_cds_seq))
        ...
        # Get TIR:
        try:
            tir = tir_vals[len(pre_cds_seq)][1]
        except KeyError:
            # If RBS cannot be found at start position, set tir to be zero:
            tir = 0

        cds['parameters'] = cds.get('paremeters', {})
        ...
        rogue_rbs = [(pos, terms)
                     ...
                     if pos != len(pre_cds_seq) and terms[1] >
...
class PartsThread(SimulatedAnnealer):
    ...
    def __init__(self, query, idx, verbose=False):
        self.solution = PartsSolution(
            query['designs'][idx],
            query.get('organism', None),
            query['filters'])

        SimulatedAnnealer.__init__(self, self.solution,
                                   verbose=verbose)
