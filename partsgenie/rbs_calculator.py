...
from partsgenie.nucl_acid_utils import NuclAcidCalcRunner
...
class RbsCalculator():
    ...
    def get_initial_rbs(self, rbs_len, cds, tir_target_rel):
        ...
        if dg_target_rel < 0.125:
            ...
            prob_shine_delgano = 0.90
            core_length = 8
            max_nonopt_spacing = 3
        else:
            prob_shine_delgano = 1.0
            core_length = 9
            max_nonopt_spacing = 2
    ...
    def __calc_dg_standby_site(self, m_rna, bp_x, bp_y, energy_before,
        ...
        d_g = min(energy_before - energy_after, 0.0)
        return d_g

    def __get_random_rbs(self, rbs_len, shine_delgano, prob_shine_delgano,
                         core_length, max_nonopt_spacing):
        ...
        spacing = random.choice(
            range(
                max(0, offset + self.__optimal_spacing - max_nonopt_spacing),
                offset + self.__optimal_spacing + max_nonopt_spacing))

        rbs.extend([random.choice(['A', 'C', 'G', 'T'])
                    for _ in range(spacing)])

        # if len(rbs) > MAX_RBS_LENGTH:
        #    rbs = rbs[len(rbs) - MAX_RBS_LENGTH:len(rbs) + 1]

        return ''.join([random.choice(['A', 'C', 'G', 'T'])
                        for _ in range(rbs_len - len(rbs))] + rbs)

    def __calc_aligned_spacing(self, m_rna, start_pos, bp_x, bp_y):
        """Calculates the aligned spacing between the 16S r_rna binding site
        and the start codon."""
