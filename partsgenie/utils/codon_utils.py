...
from partsgenie.utils import ncbi_tax_utils, seq_utils
...
class CodonOptimiser():
    ...
    def get_codon_optim_seq(self, protein_seq, excl_codons=None,
        ...
        if max_repeat_nuc == float('inf') and restr_enzyms is None:
            return ''.join([self.__get_random_codon(aa, excl_codons)
        ...
        while attempts < max_attempts:
            ...
            new_seq = seq + self.__get_random_codon(amino_acid, excl_codons)
        ...
        raise ValueError('Unable to generate codon-optimised sequence with '
                         f'{max_repeat_nuc} maximum repeating nucleotides.')
    ...
    def mutate(self, protein_seq, dna_seq, mutation_rate):
        ...
        return ''.join([self.__get_random_codon(amino_acid)
                        if random.random() < mutation_rate
                        else dna_seq[3 * i:3 * (i + 1)]
                        for i, amino_acid in enumerate(protein_seq)])

    def __get_random_codon(self, amino_acid, excl_codons=None):
...
def _read_codon_usage_orgs_file(filename):
    ...
    with open(filename, 'r', encoding=locale.getpreferredencoding()) \
            as textfile:
        next(textfile)
...
def _write_codon_usage_orgs_file(codon_orgs, filepath):
    ...
    with open(filepath, 'w+', encoding=locale.getpreferredencoding()) as fle:
        fle.write('Name\tId\n')
...
def _get_codon_usage(taxonomy_id):
    """Get codon usage"""
    curr_dir = os.path.dirname(os.path.realpath(__file__))

    filepath = os.path.join(
        curr_dir, 'codon_usage_tables', f'{taxonomy_id}.txt')

    encoding = locale.getpreferredencoding()

    if os.path.exists(filepath):
        with open(filepath, encoding=encoding) as fle:
            return fle.read()

    # else:
    url = 'http://www.kazusa.or.jp/codon/cgi-bin/showcodon.cgi?species=' \
        + str(taxonomy_id) + '&aa=1&style=GCG'

    with urllib.request.urlopen(url) as fle:
        return fle.read().decode(encoding)
